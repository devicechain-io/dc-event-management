...
func (ResourceProvider) GetConfigurationResources() ([]gen.ConfigurationResource, error) {
	...
	name := "event-management-default"
	msconfig := config.NewEventManagementConfiguration()
	content, err := gen.GenerateMicroserviceConfig(name, "event-management", "devicechain-io/event-management:0.0.1", msconfig)
	if err != nil {
		return nil, err
	}
